...
public class EventTemplateDB {
	 ...
	 * 
	 * @param esfInputStream
	 *            the InputStream representing an ESF file
	 */
	...
	public synchronized boolean addEvent(String anEventName) {
		...
		try {
			...
			if (!(reservedWords.isEmpty())) {
				...
				Iterator<String> i = reservedWords.keySet().iterator();
				while(i.hasNext()) {
	 ...
	 * 
	 * @param anEventName
	 *            the name of the event to add this attribute to
	 * @param anAttributeName
	 *            the name of the attribute to add
	 * @param anAttributeType
	 *            the type of the attribute, should be the name of the type
		...
		try {
			if (anEventName.equals(META_EVENT_INFO)) {
				if (checkForType(anAttributeType)) {
					reservedWords.put(anAttributeName, knownTypes
							.get(anAttributeType));
					return true;
				} else {
	 ...
	 * 
	 * @param aTypeName
	 *            a type name according to the ESF Specification
	 ...
	 * 
	 * @param anEventName
	 *            the name of the event to check the existence of
	 ...
	 *            the name of an Event.
	 * @param anAttributeName
	 *            the name of the attribute whose type is being checked
	 * @param anAttributeValue
	 *            the Object containing the possible value of the attribute.
	 * 
	 ...
	 *            the name of the attribute whose type is being checked
	 * @param anAttributeType
	 *            the String containing the possible type value of the
	 *            attribute.
	 * 
	 * @return true if the event and attribute exist and if the type of the
	 *         attribute matches the type assigned to this attribute in the
	 ...
	 *            the name of an Event.
	 * @param attributeName
	 *            the name of an attribute of <tt>eventName</tt>
	 ...
	 * @param stringAttributeValue
	 *            a string representation of the value of the attribute given by
	 *            anAttributeName.
	...
	public Object parseAttribute(String anEventName, String anAttributeName,
		...
		if (checkForAttribute(anEventName, anAttributeName)) {
			...
			Map<String,BaseType> evtHash = (Map<String,BaseType>) events.get(anEventName);
			try {
				BaseType bt = (BaseType) evtHash.get(anAttributeName);
				if (bt == null)
	...
	public String toHtmlString() {
		...
		for (Iterator<String> res = reservedWords.keySet().iterator(); res.hasNext();) {
			String key = (String) res.next();
			BaseType tv = (BaseType) reservedWords.get(key);
			String type = tv.getTypeName();
			sb.append("<tr><td></td><td>" + type + "</td><td>" + key
					+ "</td></tr>\n");
		}
		for (Iterator<String> iter = events.keySet().iterator(); iter.hasNext();) {
			String EventKey = (String) iter.next();
			sb.append("<tr><th>" + EventKey
					+ "</th><th>Type</th><th>Name</th></tr>\n");
			if (EventKey != null) {
				Map<String,BaseType> event = (Map<String,BaseType>) events.get(EventKey);
				for (Enumeration<String> att = Collections.enumeration(event.keySet()); att
						.hasMoreElements();) {
					String key = (String) att.nextElement();
					BaseType tv = (BaseType) event.get(key);
					String type = tv.getTypeName();
					sb.append("<tr><td></td><td>" + type + "</td><td>" + key
							+ "</td></tr>\n");
				}
	...
	public String toString() {
		...
		for (Iterator<String> res = reservedWords.keySet().iterator(); res.hasNext();) {
			reservedKeys[i] = (String) res.next();
			++i;
		}
		Arrays.sort(reservedKeys);
		
		for (i = 0; i < reservedKeys.length; ++i) {
			BaseType tv = (BaseType) reservedWords.get(reservedKeys[i]);
			String type = tv.getTypeName();
			sb.append("\t" + type + " " + reservedKeys[i] + ";\n");
		}
		sb.append("}\n");
		...
		i = 0;
		for (Iterator<String> iter = events.keySet().iterator(); iter.hasNext();) {
			eventKeys[i] = (String) iter.next();
			++i;
		}
		Arrays.sort(eventKeys);
		
		for (i = 0; i < eventKeys.length; ++i) {
			sb.append(eventKeys[i] + "\n{\n");
			if (eventKeys[i] != null) {
				Map<String, BaseType> event = (Map<String, BaseType>) events.get(eventKeys[i]);
				j = 0;
				...
				for (j = 0; j < attributeKeys.length; ++j) {
					BaseType tv = (BaseType) event.get(attributeKeys[j]);
					String type = tv.getTypeName();
					sb.append("\t" + type + " " + attributeKeys[j] + ";\n");
				}
			}
			sb.append("}\n");
		}
